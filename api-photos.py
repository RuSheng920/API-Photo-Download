...
try:
    ...
    PIL_AVAILABLE = False
    logging.warning("⚠️ 未安装Pillow库（pip install pillow），将仅校验文件头，无法验证图片完整性")
...
RETRY_STATUS_CODES = {408, 500, 502, 503, 504, 429}  # 可重试状态码
# 图片文件头特征（用于快速校验）
IMAGE_MAGIC_NUMBERS = {
...
def download_image_chunked(url, timeout):
    ...
    try:
        ...
        if response.status_code in RETRY_STATUS_CODES:
            log.warning(f"⚠️  遇到可重试状态码：{response.status_code}，将重试")
            return None
        ...
        if not image_content:
            log.warning(f"⚠️  下载内容为空 | URL={url}")
            return None
        
        return bytes(image_content)
    
    except requests.exceptions.ConnectTimeout:
        log.warning(f"⚠️  连接超时（{timeout[0]}秒）| URL={url}")
        return None
    except requests.exceptions.ReadTimeout:
        log.warning(f"⚠️  读取超时（{timeout[1]}秒）| URL={url}（慢加载？）")
        return None
    except requests.exceptions.HTTPError as e:
        if e.response.status_code in RETRY_STATUS_CODES:
            log.warning(f"⚠️  HTTP错误 {e.response.status_code} | URL={url}，将重试")
            return None
        log.error(f"❌ 不可重试HTTP错误 {e.response.status_code} | URL={url}")
        return None
    except requests.exceptions.RequestException as e:
        log.warning(f"⚠️  请求异常 | 错误={e} | URL={url}")
        return None
    except Exception as e:
...
def download_images(url, directory, num_threads, total_downloads):
    ...
    with tqdm(total=total_downloads, desc="📥 下载进度", unit="次") as pbar:
        ...
        # 构建任务列表：总共total_downloads个任务
        tasks = [url] * total_downloads
        
        # 线程池执行任务：用num_threads个线程，完成total_downloads个任务
...
def main():
    ...
    if not PIL_AVAILABLE:
        print("\n⚠️  建议安装Pillow库以启用完整的图片完整性校验：pip install pillow\n")
